...
Since the dawn of mankind, human relationships have been built from reoccurring experiences. The measure of how frequent, and how positive or negative interactions are, shape what we call trust between parties of any relationship, which function constructively through reciprocity. 
...
_What if time itself was the balance for a composite weighting system?_

This is exactly the basis behind a new voting model for capital markets we designed known as [Polycentric Voting](https://focal.org/polycentric-voting.pdf), where time is introduced as an additional domain for authority past capital. 

Following in precedent to the lessons of past attempts, we introduce the concept of **effective time ($t_{\text{effective}}$)**; a capital-weighted time metric, resulting in dynamics where the time weight is rebalanced on subsequent deposits and preserved on reductions. 
...
Seeking to adopt tenure in the modelling of voting power, we propose a novel weighting mechanism; a composite weight composed of root and linear ruling that deviates with time to purely linear. Where the proportion of linear to root is expressed as the **power ratio ($\alpha$)**, where a higher value translates to  saturating larger actors influence and provide broader representation of the sum parts to those of the smaller. 
...
[^8]: A. Tang, Sheau-Tyng Peng, *Quadratic Voting and Common Ownership Self-Assessed Tax (2019)*
